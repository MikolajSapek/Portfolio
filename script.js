...
const chapters = {
    ...
    'hidden-tides': {
        ...
        photos: getPhotosFromFolder('korea', [
            '7a3cef52-45b1-404d-bb94-ef75b4867118.jpg',
            'ac7be8b6-81fd-4f99-9ccc-96a095b316f0.jpg',
            'IMG_3352.jpg',
            'IMG_3354.jpg',
            'IMG_3355.jpg',
            'IMG_3356.jpg',
...
function initAlbumsPreview() {
    ...
    albumsMarkup += `
        <a href="#horizontal-gallery-section" class="album-item" data-album="collections" data-index="${currentIndex}">
            Namibia <span class="album-count">(30)</span>
        </a>
    `;
...
function initHorizontalGallery() {
    ...
    const albumsHTML = horizontalAlbums.map((album, panelIndex) => {
        ...
        if (panelIndex === 0) {
            ...
            // On last panel, increase container sizes and shift photos to the left
            if (isLastPanel && !hasSpecialPhotos) {
                // Shift all photos to the left by reducing left values
                const leftShift = 10; // Shift 10% to the left
                // Increase all sizes by ~30-40%
                layout = {
                    main: {
                        left: Math.max(parseFloat(layout.main.left) - leftShift, 0) + '%',
                        top: layout.main.top,
                        width: Math.min(parseFloat(layout.main.width) * 1.35, 95) + '%',
                        height: Math.min(parseFloat(layout.main.height) * 1.35, 85) + '%'
                    },
                    small1: {
                        left: Math.max(parseFloat(layout.small1.left) - leftShift, 0) + '%',
                    ...
                    small2: {
                        left: Math.max(parseFloat(layout.small2.left) - leftShift, 0) + '%',
